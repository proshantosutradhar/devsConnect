...
function Feed() {
    ...
    const getFeed = async()=>{ 
        try{
        const res = await axios.get(BASE_URL + '/user/feed?page=1&limit=200',{withCredentials: true})
      ...
      const firstUser = feed?.[0]
       if(feed.length === 0) {
        return <div className="flex w-full h-[88vh] justify-center" style={{ backgroundImage: `url(${feedBanner})`, backgroundSize: "cover",
  ...
  return (
    ...
    feed &&<div className='flex w-full h-[88vh] justify-center items-center' style={{ backgroundImage: `url(${feedBanner})`, backgroundSize: "cover",
      backgroundPosition: "center" }}>
            {firstUser ? (
